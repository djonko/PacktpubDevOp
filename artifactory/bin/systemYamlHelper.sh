...
getYamlValue(){
    ...
    if "${YQ}" r "${yamlFile}" "${key}" >/dev/null 2>&1; then
        YAML_VALUE=$("${YQ}" r "${yamlFile}" "${key}" 2>/dev/null)
    else
        YAML_VALUE=
        return
    fi
...
removeYamlValue(){
    ...
    "${YQ}" d -i "${yamlFile}" "${key}" 2>/dev/null
}
...
setSystemValue(){
    ...
    if [[ ! -f "${ymlFile}" || ! -s "${ymlFile}" ]]; then
        ${YQ} n ${option} "${path}" "${value}" > "${ymlFile}" || errorExit "failed to set create ${path} with ${displayValue} in ${yamlFile}, make sure its parent directory is already create and $USER user has permission to create this file under it,\ncommand used : ${YQ} n \"${path}\" \"${displayValue}\" > \"${ymlFile}\""
    else
        ${YQ} w -i ${option} "${ymlFile}" "${path}" "${value}" || errorExit "failed to set create ${path} with ${displayValue} in ${yamlFile}, \ncommand used : ${YQ} w -i \"${ymlFile}\" \"${path}\" \"${displayValue}\""
    fi
}
