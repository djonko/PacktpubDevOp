...
setPostgresVersion() {
    ...
    case "$PRODUCT_NAME" in
        ...
        $DISTRIBUTION_LABEL)
            ...
            JFROG_POSTGRES_VERSIONS=("9.6.10" "10.13" "12.3" "12.5")
        ;;
        $XRAY_LABEL)
            JFROG_POSTGRES_9X_VERSION="9-5-2v"
            # To bump up postgresql in xray add new version in array
            JFROG_POSTGRES_VERSIONS=("9.5.2" "10.13" "12.3" "12.5")
        ;;
    esac
}
...
migrateResolveHaDirPath () {
    ...
    if [[ "${INSTALLER}" == "${RPM_TYPE}" || "${INSTALLER}" == "${COMPOSE_TYPE}" || "${INSTALLER}" == "${DOCKER_TYPE}" || "${INSTALLER}" == "${DEB_TYPE}" ]]; then
...
checkArtifactoryVersion () {
    ...
    elif [[ "${INSTALLER}" ==  "${ZIP_TYPE}" || "${INSTALLER}" == "${DOCKER_TYPE}" ]]; then
...
checkForTlsConnector () {
    ...
    do
        ...
        local sslEnabledProtocols=$($LIBXML2_PATH --xpath '//Server/Service/Connector['$i']/@sslEnabledProtocols' ${filePath}/${fileName} 2>/dev/null | awk -F"=" '{print $2}' | tr -d '"')
        if [[ "${sslProtocolValue}" == *"TLS"* || "${sslEnabledProtocols}" == *"TLS"* ]]; then
...
replicatorSetDefaultPort () {
    local sourceFile="$1"
    local key="$2"
    local internalUrlValue="$3"

    if [ "${key}" != "internalUrl" ]; then
        return
    fi
    if [ -z "${internalUrlValue}" ]; then
        return
    fi
    getYamlValue "listenPort" "${sourceFile}" "false"
    local portValue="${YAML_VALUE}"
    if [[ -z "${portValue}" ]]; then
        setSystemValue "replicator.port" "6061" "${SYSTEM_YAML_PATH}"
        logger "Setting [replicator.port] with default port [6061] in system.yaml"
    fi
}

replicatorProfiling () {
    local key="$1"
    local profileValue="$2"

    if [[ "${key}" != "profilingDisabled" ]]; then
        return
    fi
    if [[ -z "${profileValue}" ]]; then
        return
    fi
    [[ "${profileValue}" == "false" ]] && value="true"
    [[ "${profileValue}" == "true" ]]  && value="false"
}
...
_createBackupOfLogBackDir () {
    ...
    # Only for docker migration delete logback.xml under etc directory, because etc directory is symlinked 
    if [[ "${INSTALLER}" == "${DOCKER_TYPE}" ]]; then
        rm -f "${NEW_DATA_DIR}/etc/logback.xml" || warn "Failed to remove file [${NEW_DATA_DIR}/etc/logback.xml]"
    fi
}
...
moveConfigFiles() {
    _moveConfigFilesUnderOldDir "${NEW_DATA_DIR}/etc/artifactory/db.properties" "${NEW_DATA_DIR}/etc/artifactory/old"
    _moveConfigFilesUnderOldDir "${NEW_DATA_DIR}/etc/artifactory/ha-node.properties" "${NEW_DATA_DIR}/etc/artifactory/old"
    _moveConfigFilesUnderOldDir "${NEW_DATA_DIR}/etc/artifactory/default" "${NEW_DATA_DIR}/etc/artifactory/old"
    _moveConfigFilesUnderOldDir "${NEW_DATA_DIR}/etc/access/db.properties" "${NEW_DATA_DIR}/etc/access/old"
    _moveConfigFilesUnderOldDir "${NEW_DATA_DIR}/etc/replicator/replicator.yaml" "${NEW_DATA_DIR}/etc/replicator/old"
}
